...
*   **本人姓名**：丁学郅
*   **本人学号**：20236486
*   **项目名称**：面向本地工具调用与记忆增强的 Agent 智能体系统
*   **实际完成目标**：包含进阶挑战项
*   **小组其他成员**：王丽莎、邓雅文、贾培鑫
...
| 组长 | 王丽莎 | 20216434 | B1 Agent 运行与消息管理；B5 记忆文档存储与查找 | (https://github.com/DylanPriestley744/agent-b1-b5-state-aware-runtime) | 
| **组员** | **丁学郅（本人）** | **20236486** | **B2: Skill工具函数** |(https://github.com/259257552/agent-b2-skill) | 
| 组员 | 邓雅文 | 20236448|B4: Agent LLM决策模块 + B1（与B4关联部分）+ 全系统 Web UI 集成 | https://github.com/YawenDeng1212/NLP-Agent-B4-LLM |
| 组员 | 贾培鑫 | 20236548 | B3：说明生成与工具调用模块 | [https://github.com/Blue-point-point/personal_b3](https://github.com/Blue-point-point/personal_b3) | 
...
![图1 多工具协同执行](images/demo_multi_tool_flow.png)

*用户输入"在docs目录搜索所有txt文件，并读取第一个，计算150*28"，系统依次调用local_file_search、file_reader、calculator三个Skill，B1调度完成4步Plan执行，B5记忆保存对话上下文。*

**图2：多轮对话与工具调用结果展示**

![图2 多轮对话与工具调用结果展示](images/demo_multi_turn_result.png)

*第一轮：搜索、读取、计算完成，返回文档内容和计算结果150*28=4200。第二轮：用户追问"计算结果加上10"，Agent利用B5记忆获取上一轮结果4200，直接计算4200+10=4210，无需重新调用工具。*

**图3：记忆感知与上下文延续**

![图3 记忆感知与上下文延续](images/demo_memory_context.png)

*B5记忆模块加载历史对话（2条），B1调度识别为plan_execute模式，B4生成执行计划，展示Agent如何利用记忆实现多轮上下文延续。*

**图4：推理配置与记忆设置**

![图4 推理配置与记忆设置](images/demo_config_memory.png)

*右侧推理配置面板：Native Tools模式、Auto循环、全局记忆开启、关键词匹配检索、保存到记忆开启。展示B5记忆模块的可配置性与B1历史压缩选项。*

**图5：Agent思考过程与模块协作**

![图5 Agent思考过程与模块协作](images/demo_agent_thinking.png)

*Agent思考中状态：B5记忆加载(0.0s) → B1调度任务分类(9.6s) → B1启动Plan & Execute(9.6s) → B4生成执行计划(0.0s)。完整展示B1-B4-B5的协作时序。*

### 2.4 团队系统代码库

*   **团队 Github/Gitee 开源仓库链接**: (https://github.com/DylanPriestley744/8group_B_Agent)
